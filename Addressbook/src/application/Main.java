...
public class Main extends Application{
	...
	try {
		...
		// Grid, textfields and labels attributes
			...
			try {
				...
				if(db.length() != 0) {
				  ...
				  if(p.get(p.size()-1)!=null) {
					  ...
					  fnew.addDatabase(id, tfName.getText(), tfStreet.getText(), tfCity.getText(), tfGender.getText(),tfZip.getText(),db);
					  } 
			    }
				else {
					 p.add(new Person(0, tfName.getText(), tfStreet.getText(), tfCity.getText(),tfGender.getText(),Integer.parseInt(tfZip.getText())));
						  ...
						  FileOps fnew=new FileOps();
						  fnew.addDatabase(0, tfName.getText(), tfStreet.getText(), tfCity.getText(), tfGender.getText(),tfZip.getText(),db);					
				}
				
				Alert inf =new Alert(AlertType.CONFIRMATION);
				inf.setTitle("Info");
				inf.setHeaderText("Record is added succesfully.");
				inf.show();
			}
				...
				inf.setTitle("Info");
				inf.setHeaderText("Please record correctly");
				inf.show();
			    }
...
btFirst.setOnAction(e->{
	try {
		...
		btUbyID.setOnAction(e->{
			...
			update.updateDatabase(id, db, updaterow);
			}
			catch(Exception e2) {
